		 ...
		 train_db.open(function(err, train_db) {
		 	...
		 	async.waterfall([   //time to do things one at a time
			 	...
			 	function(callback) {  //CREATE CLASSIFIERS
					...
					//var the_taglist = ["c#","java","php","javascript","android","jquery","c++","python","iphone","asp.net","mysql","html",".net","ios","objective-c","sql","css","linux","ruby-on-rails","windows","c","sql-server","ruby","wpf","xml","ajax","database","windows-7","asp.net-mvc","regex"];
			 ...
			 /*		var natural = require('natural'),
				...
				callback(null, title_tag_classifier, body_tag_classifier);
			 	...
			 	function(title_tag_classifier, body_tag_classifier, callback) {   //SAVE CLASSIFIERS
			 		...
			 		var add_keyword_search_list = require('../classifiers/bayes_addon_search_tags.js');
			 		add_keyword_search_list = add_keyword_search_list.thetags;


					var stream = fs.createWriteStream('../../Data/test_superbayes_TRAIN_TEST.csv', {flags: 'w'}); //create write stream and point towards output file
						...
						//	console.log("BODY" + body);						
							...
							async.waterfall([   //time to do things one at a time
								...
								function(callback) {    				//PROCESS TEST DATA
									...
									//console.log("TEST CLASSIFY: " + tag_classifier.getClassifications("fdafdasfdsagrwagf grawdg dwag awddfg adg javascript php"));

									var classify_body_results = body_tag_classifier.getClassifications(body);
									var classify_title_results = title_tag_classifier.getClassifications(title);

								//	console.log(  "CLASSIFIER TEST RUN: " + body_tag_classifier.classify(combined_text)  );

								/*	for(var j = 0; j < classify_title_results; j++){
										for(var i = 0; i < classify_body_results; i++){
											if(classify_title_results[j].Tag === classify_body_results[i].Tag) classify_title_results[j].Num = classify_title_results[j].Num + classify_body_results[i].Num;
									...
									//var classify_results = classify_title_results; //.concat(classify_body_results);

									if(classify_title_results !== undefined){

										for(var n = 0; n < classify_title_results.length; n++){  
											total_score = total_score +  classify_title_results[n].value;
											if(classify_title_results[n].label === "gridview" || classify_title_results[n].label === "asp.net"  )	classify_title_results[n].value = classify_title_results[n].value/2;
										}

										var title_average_score = total_score / classify_title_results.length;

									//	var title_average_score = 0.00005; //FOR TESTING**************************

										console.log("AVERAGE: " + title_average_score);

										console.log("TAG: " + classify_title_results[3].label + "SCORE: " + classify_title_results[3].value/title_average_score);	

										for(var z = 0; z < classify_title_results.length; z++){


											if( (classify_title_results[z].value / title_average_score) > title_cutoff){

												found_tags.push(classify_title_results[z].label   + (classify_title_results[z].value/title_average_score).toString()   );	//LAST BIT ADDED FOR TESTING
												console.log("TITLE TAG ADDED: " + classify_title_results[z].label + "SCORE: " + classify_title_results[z].value/title_average_score);	
											}
									...
									if(classify_body_results !== undefined){

										total_score=0;

										for(var n = 0; n < classify_body_results.length; n++){  
											total_score = total_score +  classify_body_results[n].value;
											if(classify_body_results[n].label === "gridview" || classify_body_results[n].label === "asp.net"  )	classify_body_results[n].value = classify_body_results[n].value/3;	
										}

										var body_average_score = total_score/classify_body_results.length;

										//var body_average_score = 0.00005; //FOR TESTING**************************

											console.log("AVERAGE: " + body_average_score);

										for(var r = 0; r < classify_body_results.length; r++){

											if( (classify_body_results[r].value/body_average_score) > body_cutoff && found_tags.indexOf(classify_body_results[r].label) === (-1)){

												found_tags.push(classify_body_results[r].label          +  (classify_body_results[r].value/body_average_score).toString()            );	  //LAST BIT ADDED FOR TESTING

												console.log("BODY TAG ADDED: " + classify_body_results[r].label + "SCORE: " + classify_body_results[r].value/body_average_score);	
											}
									...
									if(found_tags.length > 5){
									   ...
									   found_tags = found_tags.slice(0,5);
										...
										for(var h = 0; h < add_keyword_search_list.length;h++){
											if(title.indexOf(" " + add_keyword_search_list[h] + " ") !== (-1) && tagstop(add_keyword_search_list[h]) === true) found_tags.push(add_keyword_search_list[h]);
										}
										for(var q = 0; q < select_tags_raw.length;q++){
											if(body.indexOf(" " + select_tags_raw[q] + " ") !== (-1) && tagstop(select_tags_raw[q]) === true) found_tags.push(select_tags_raw[q]);
										}
										found_tags = found_tags.slice(0,5);
									}


									//found_tags.push(classify_body_results[3].label);	
									//found_tags.push(classify_title_results[3].label);
									//found_tags.push("test3");


									
									/*

									if(classify_results !== undefined){


										for(var n = 0; n < classify_results.length; n++){  total_score = total_score +  classify_results[n].value;	}
										average_score = total_score/classify_results.length; */


										

									/*    if(classify_results[0].value/average_score > score_cutoff1) found_tags.push(classify_results[0].label);	
									    if(classify_results[1].value/average_score > score_cutoff1) found_tags.push(classify_results[1].label);	
									    if(classify_results[2].value/average_score > score_cutoff2) found_tags.push(classify_results[2].label);	
									    if(classify_results[3].value/average_score > score_cutoff2) found_tags.push(classify_results[3].label);	
									    if(classify_results[4].value/average_score > score_cutoff2) found_tags.push(classify_results[4].label);	
